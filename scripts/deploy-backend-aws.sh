...
runtime_json="$(node -e 'const [raw,market,token,cert,rpc]=process.argv.slice(1); const x=JSON.parse(raw); Object.assign(x,{COURSE_MARKET_ADDRESS:market,YD_TOKEN_ADDRESS:token,CERT_NFT_ADDRESS:cert,WORKER_MARKET_ADDRESSES:market,WORKER_RPC_URLS:rpc}); process.stdout.write(JSON.stringify(x))' "$runtime_json" "${VITE_COURSE_MARKET_ADDRESS:-}" "${VITE_YD_TOKEN_ADDRESS:-}" "${VITE_CERTIFICATE_NFT_ADDRESS:-}" "${VITE_SEPOLIA_RPC_URL}")"
