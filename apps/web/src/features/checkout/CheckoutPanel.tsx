...
export function CheckoutPanel({ courseId, courseTitle, priceYD, courseKey, recipient, walletId, generateIdempotencyKey, onSuccess, defaultTermsAccepted = false }: CheckoutPanelProps) {
  const [accepted, setAccepted] = useState(defaultTermsAccepted);
  const [errorBanner, setErrorBanner] = useState<string | null>(null);

  const breakdown = useMemo(() => computeBreakdown(priceYD), [priceYD]);
  ...
  return (
    <section className='checkout-panel panel' aria-labelledby='checkout-panel-title'>
      ...
      <label className='checkout-panel__terms' style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 0' }}>
        <input style={{ flex: 0 }} type='checkbox' checked={accepted} onChange={(e) => setAccepted(e.target.checked)} />
        <span>我已知晓链上购买不可撤销且不予退款，课程解锁与当前连接的钱包绑定。</span>
      </label>
